...
\end{itemize}\end{frame} \begin{frame}[allowframebreaks] \frametitle{$A_{\text{TM}}$}
\begin{equation*}
  A_{\text{TM}}=\{
\langle  M,w\rangle \mid M: \mbox{ a TM that accepts } w\}
\end{equation*}
  \begin{itemize}
\item We will prove that $A_{\text{TM}}$ is undecidable
\item However, $A_{\text{TM}}$ is Turing recognizable
...
\begin{tikzpicture}
  ...
  \draw[-][scale=0.5, domain=-1.7:1.7, smooth, variable=\x, blue] plot ({\x}, {\x*\x*\x});
\end{tikzpicture}
\end{center}
